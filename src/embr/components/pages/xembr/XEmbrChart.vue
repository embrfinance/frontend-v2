...
import { GqlxEmbrDataSnapshot } from '@/embr/services/embr/embr-types';
import BalBtn from '@/components/_global/BalBtn/BalBtn.vue';
import { capitalize } from 'lodash';

type Props = {
  snapshots: GqlxEmbrDataSnapshot[];
};
...
type ChartType = 'weekly' | 'totals';
const currentChartType = ref<ChartType>('weekly');
    ...
    series: [
      {
        ...
        data: props.snapshots.map(snapshot => {
          switch (currentChartType.value) {
            case 'weekly':
              return parseFloat(snapshot.rewardValue1w);
            case 'totals':
              return parseFloat(snapshot.rewardValueTotal);
          }
        }),
      ...
      <div class="flex justify-end">
        <div class="toggle-container">
          <div
            :class="[
              'toggle-button',
              currentChartType === 'weekly' ? 'toggle-button-active' : ''
            ]"
            @click="setCurrentChart('weekly')"
          >
            Weekly
          </div>
          <div
            :class="[
              'toggle-button',
              currentChartType === 'totals' ? 'toggle-button-active' : ''
            ]"
            @click="setCurrentChart('totals')"
          >
            Totals
          </div>
        </div>
