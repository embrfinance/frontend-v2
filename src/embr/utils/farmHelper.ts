...
export function calculateApr(
  ...
  const valuePerYear =
    embrPrice * farmBeetsPerYear + rewardTokenPrice * rewardTokenPerYear;
...
export function getPoolApr(
  ...
  const liquidityMiningApr = farm
    ? `${calculateApr(farm, tvl, blocksPerYear, embrPrice, rewardTokenPrice)}`
    : '0';
...
export function decorateFarm(
  ...
  return {
    ...
    pendingBeetsValue: (farmUser?.pendingBeets || 0) * embrPrice,
    share: userShare,
